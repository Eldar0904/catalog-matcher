...
1. Upload `Government_Catalog.xlsx` (columns: Government Code, Product Name,
   ...
   names are matched loosely, so small variations are fine).
...
5. Click "Export Offer.xlsx" to download the final results.
...
backend/app/
  ...
  services/
    ...
    export.py                 # Offer.xlsx generation
...
pip install -r requirements.txt
export DATABASE_URL=postgresql://postgres:postgres@localhost:5432/catalog_matcher
python -m uvicorn app.main:app --reload
```
